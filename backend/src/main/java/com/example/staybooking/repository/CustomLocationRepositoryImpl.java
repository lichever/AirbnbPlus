...
public class CustomLocationRepositoryImpl implements CustomLocationRepository {
  ...
  public List<Long> searchByDistance(double lat, double lon, String distance) {
    ...
    if (distance == null || distance.isEmpty()) {//string from ui
      distance = DEFAULT_DISTANCE;

    }
    //this is ES query; if this is sql db, we can use @Query in StayReservationDateRepository example
    NativeSearchQueryBuilder queryBuilder = new NativeSearchQueryBuilder();
    queryBuilder.withFilter(new GeoDistanceQueryBuilder("geoPoint").point(lat, lon)
                                                                   .distance(distance,
                                                                       DistanceUnit.KILOMETERS));

    SearchHits<Location> searchResult = elasticsearchOperations.search(queryBuilder.build(),
        Location.class);
